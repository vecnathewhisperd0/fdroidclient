...
class NotificationHelper {
    ...
    private boolean shouldIgnoreEntry(AppUpdateStatusManager.AppUpdateStatus entry) {
        ...
        return entry.status == AppUpdateStatusManager.Status.DownloadInterrupted || (entry.status == AppUpdateStatusManager.Status.Downloading || entry.status == AppUpdateStatusManager.Status.ReadyToInstall || entry.status == AppUpdateStatusManager.Status.InstallError) && AppDetails2.isAppVisible(entry.app.packageName);
    }
