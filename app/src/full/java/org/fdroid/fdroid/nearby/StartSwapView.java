...
public class StartSwapView extends SwapView {
    ...
    private void uiInitPeers() {
        ...
        peopleNearbyList.setOnItemClickListener((parent, view, position, id) ->
                onPeerSelected(peopleNearbyAdapter.getItem(position)));
    }
    ...
    private void uiInitBluetooth() {
        if (bluetooth != null && ContextCompat.checkSelfPermission(getContext(),
            ...
            viewBluetoothId = findViewById(R.id.device_id_bluetooth);
    ...
    private final CompoundButton.OnCheckedChangeListener onBluetoothSwitchToggled =
            (buttonView, isChecked) -> {
                if (isChecked) {
                    Utils.debugLog(TAG, "Received onCheckChanged(true) for Bluetooth swap, " +
                            "prompting user as to whether they want to enable Bluetooth.");
                    getActivity().startBluetoothSwap();
                    textBluetoothVisible.setText(R.string.swap_visible_bluetooth);
                    viewBluetoothId.setVisibility(View.VISIBLE);
                    Utils.debugLog(TAG, "Received onCheckChanged(true) for Bluetooth swap " +
                            "(prompting user or setup Bluetooth complete)");
                    // TODO: When they deny the request for enabling bluetooth, we need to disable
                    //  this switch...
                } else {
                    Utils.debugLog(TAG, "Received onCheckChanged(false) for Bluetooth swap, " +
                            "disabling Bluetooth swap.");
                    ...
                    Utils.debugLog(TAG, "Received onCheckChanged(false) for Bluetooth swap, " +
                            "Bluetooth swap disabled successfully.");
                }
